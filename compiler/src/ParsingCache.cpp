...
bool ParsingCache::containsWildcard(std::string const& filename){
    ...
    auto equivalentFind = std::find_if(filenamesToAlreadyImportedIdentifiers.begin(), filenamesToAlreadyImportedIdentifiers.end(), [&](auto const& other){
    ...
    if (equivalentFind != filenamesToAlreadyImportedIdentifiers.end()){
        return containsWildcard((*equivalentFind).first);
    }
