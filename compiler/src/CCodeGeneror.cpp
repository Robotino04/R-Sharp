...
void CCodeGenerator::visit(std::shared_ptr<AstForLoopExpression> node) {
    ...
    node->increment->accept(this);
    emit(") ");
    if (node->body->getType() == AstNodeType::AstBlock) node->body->accept(this);
    else {
        emit("{\n");
        indent();
        node->body->accept(this);
...
void CCodeGenerator::visit(std::shared_ptr<AstUnary> node) {
    emit("(");
    switch (node->type) {
        case AstUnaryType::Negate:     emit("-"); break;
        case AstUnaryType::LogicalNot: emit("!"); break;
        case AstUnaryType::BinaryNot:  emit("~"); break;
        default:                       Fatal("Invalid unary operator Nr. ", static_cast<int>(node->type));
    }
    node->value->accept(this);
    emit(")");
}
void CCodeGenerator::visit(std::shared_ptr<AstBinary> node) {
    ...
    switch (node->type) {
        case AstBinaryType::Add:                emit(" + "); break;
        case AstBinaryType::Subtract:           emit(" - "); break;
        case AstBinaryType::Multiply:           emit(" * "); break;
        case AstBinaryType::Divide:             emit(" / "); break;
        case AstBinaryType::Modulo:             emit(" % "); break;

        case AstBinaryType::Equal:              emit(" == "); break;
        case AstBinaryType::NotEqual:           emit(" != "); break;
        case AstBinaryType::LessThan:           emit(" < "); break;
        case AstBinaryType::LessThanOrEqual:    emit(" <= "); break;
        case AstBinaryType::GreaterThan:        emit(" > "); break;
        case AstBinaryType::GreaterThanOrEqual: emit(" >= "); break;

        case AstBinaryType::LogicalAnd:         emit(" && "); break;
        case AstBinaryType::LogicalOr:          emit(" || "); break;

        default:                                Fatal("Invalid binary operator Nr. ", static_cast<int>(node->type));
    }
...
void CCodeGenerator::visit(std::shared_ptr<AstVariableDeclaration> node) {
    ...
    if (node->value) {
        ...
        if (!node->variable->isGlobal) {
            emit(
                "; zero_memory_r_sharp_internal(&" + node->name + ", "
                + std::to_string(sizeFromSemanticalType(node->semanticType)) + ")"
            );
        }
        else {
...
void CCodeGenerator::visit(std::shared_ptr<AstPrimitiveType> node) {
    switch (node->type) {
        ...
        default:                          Fatal("Unimplemented type Nr.", static_cast<int>(node->type));
    }
}
